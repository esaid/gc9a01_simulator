...
- **Rendering Pipeline**:
  1. Drawing operations update a rectangular PIL image.
  2. `update_display()` or `renderToTk()` applies a circular mask.
  3. The resulting image is rotated based on the current rotation setting.
  4. The final image is displayed on a `tkinter.Canvas`.
