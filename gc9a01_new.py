...
class SimulatedGC9A01:
    ...
    def _apply_rotation(self, x, y):
        """Applique la rotation aux coordonnées"""
        if self.rotation == 0:
            return x, y
        elif self.rotation == 1:  # 90° sens horaire
            return self.height - y - 1, x
        elif self.rotation == 2:  # 180°
            return self.width - x - 1, self.height - y - 1
        elif self.rotation == 3:  # 270° sens horaire (ou 90° sens anti-horaire)
            return y, self.width - x - 1
        return x, y

    def setRotation(self, rotation):
        """Définit la rotation de l'écran (0, 1, 2, ou 3)"""
        self.rotation = rotation % 4

    # Modifiez vos méthodes de dessin existantes pour utiliser _apply_rotation :

    def setCursor(self, x, y):
        # Appliquer la rotation avant de vérifier les limites du cercle
    ...
    def drawPixel(self, x, y, color):
        x, y = self._apply_rotation(x, y)  # Ajoutez cette ligne
        if isinstance(color, int):
            color = self.color565_to_rgb(color)
        self.draw.point((x, y), fill=color)

    def drawLine(self, x0, y0, x1, y1, color):
        x0, y0 = self._apply_rotation(x0, y0)  # Ajoutez ces lignes
        x1, y1 = self._apply_rotation(x1, y1)
        if isinstance(color, int):
            color = self.color565_to_rgb(color)
        self.draw.line([x0, y0, x1, y1], fill=color)
    ...
    # Modifications à apporter à votre classe SimulatedGC9A01
    ...
    def printText(self, text):
        ...
        for c in text:
            if c == '\n':
                ...
                self._draw_rotated_char(c, font)

    def _draw_rotated_char(self, char, font):
        """Dessine un caractère en tenant compte de la rotation"""

        if self.rotation == 0:
            # Normal - pas de rotation
            self.draw.text((self.cursor_x, self.cursor_y), char, font=font, fill=self.text_color)
            self.cursor_x += self.text_size * 6

        elif self.rotation == 1:
            # 90° sens horaire - texte vertical vers le bas
            # Créer une image temporaire pour le caractère
            char_img = Image.new('RGBA', (20, 20), (0, 0, 0, 0))
            char_draw = ImageDraw.Draw(char_img)
            char_draw.text((0, 0), char, font=font, fill=self.text_color)

            # Faire pivoter l'image du caractère de 90°
            rotated_char = char_img.rotate(-90, expand=True)

            # Coller le caractère pivoté sur l'image principale
            self.image.paste(rotated_char, (self.cursor_x, self.cursor_y), rotated_char)
            self.cursor_y += self.text_size * 8  # Avancer vers le bas

        elif self.rotation == 2:
            # 180° - texte à l'envers
            char_img = Image.new('RGBA', (20, 20), (0, 0, 0, 0))
            char_draw = ImageDraw.Draw(char_img)
            char_draw.text((0, 0), char, font=font, fill=self.text_color)

            rotated_char = char_img.rotate(180, expand=True)
            self.image.paste(rotated_char, (self.cursor_x - rotated_char.width, self.cursor_y - rotated_char.height),
                             rotated_char)
            self.cursor_x -= self.text_size * 6  # Avancer vers la gauche

        elif self.rotation == 3:
            # 270° sens horaire (90° anti-horaire) - texte vertical vers le haut
            char_img = Image.new('RGBA', (20, 20), (0, 0, 0, 0))
            char_draw = ImageDraw.Draw(char_img)
            char_draw.text((0, 0), char, font=font, fill=self.text_color)

            rotated_char = char_img.rotate(90, expand=True)
            self.image.paste(rotated_char, (self.cursor_x - rotated_char.width, self.cursor_y), rotated_char)
            self.cursor_y -= self.text_size * 8  # Avancer vers le haut

    def _handle_newline(self):
        """Gère les retours à la ligne selon la rotation"""
        line_height = 8 * self.text_size
        ...
        if self.rotation == 0:
            ...
            # 90° - nouvelle "ligne" = nouvelle colonne vers la droite
            self.cursor_x += line_height
            self.cursor_y = 0
        elif self.rotation == 2:
            # 180° - nouvelle ligne vers le haut
            self.cursor_y -= line_height
            self.cursor_x = self.width
        elif self.rotation == 3:
            # 270° - nouvelle "ligne" = nouvelle colonne vers la gauche
            self.cursor_x -= line_height
            self.cursor_y = self.height

    def setCursor(self, x, y):
        """Définit la position du curseur (coordonnées dans le système de rotation actuel)"""
        # Ne PAS appliquer _apply_rotation ici car x,y sont déjà dans le système de coordonnées de la rotation
        r = self.width // 2
        cx, cy = r, r

        # Vérifier si on est dans le cercle (dans le système non-tourné)
        actual_x, actual_y = self._apply_rotation(x, y)
        dist = ((actual_x - cx) ** 2 + (actual_y - cy) ** 2) ** 0.5

        if dist > r:
            # Si hors du cercle, repositionner sur le bord
            angle = math.atan2(actual_y - cy, actual_x - cx)
            self.cursor_x = int(cx + r * math.cos(angle))
            self.cursor_y = int(cy + r * math.sin(angle))
            # Reconvertir en coordonnées de rotation
            self.cursor_x, self.cursor_y = self._reverse_rotation(self.cursor_x, self.cursor_y)
        else:
            self.cursor_x = x
            self.cursor_y = y

    def _reverse_rotation(self, x, y):
        """Applique la rotation inverse pour convertir des coordonnées absolues vers le système de rotation"""
        if self.rotation == 0:
            return x, y
        elif self.rotation == 1:
            return y, self.width - x - 1
    ...
    # Version alternative plus simple mais moins précise :
    def printText_simple(self, text):
        """Version simplifiée qui utilise la rotation PIL"""
        text = str(text)

        # Créer une image temporaire pour tout le texte
        ...
        # Dessiner le texte sur l'image temporaire (sans rotation)
        temp_draw.text((0, 0), text, font=font, fill=self.text_color)

        # Faire pivoter toute l'image temporaire
        if self.rotation == 1:
        ...
        # Coller l'image pivotée à la position du curseur
        self.image.paste(temp_img, (self.cursor_x, self.cursor_y), temp_img)
